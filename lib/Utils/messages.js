...
const generateWAMessageContent = async (message, options) => {
    ...
    if (hasNonNullishProperty(message, "text")) {
    ...
    } else if (hasNonNullishProperty(message, 'groupStatusMessage') || hasNonNullishProperty(message, 'groupStatusMessageV2')) {
        const key = 'groupStatusMessageV2' in message ? 'groupStatusMessageV2' : 'groupStatusMessage'
        const messageSecret = options.messageSecret || randomBytes(32)

        const inside = await generateWAMessageContent(message[key], {
    ...
    } else if (hasNonNullishProperty(message, 'bottomSheet')) {
        const {
            title = '',
            header = '',
            footer = '',
        ...
        let mediaContent = {}
        if (image) {
            mediaContent = await prepareWAMessageMedia({ image }, options)
        } else if (video) {
            mediaContent = await prepareWAMessageMedia({ video }, options)
        }
        const allButtons = [
            {
                name: 'single_select',
            ...
            nativeFlowMessage: {
                ...
                messageParamsJson: JSON.stringify({
                    bottom_sheet: {
                        ...
                        list_title: sheetTitle,
                        button_title: sheetButton
                    }
                })
            }
        }
